			...
			<table class="table table-bordered datatable" id="table-1">
				<thead>
					<tr>
						...
						<th>State</th>
						<th>Zip Code</th>
						<th>Rental Price (US Dollars)</th>
						<th>Status</th>
						<th>Manage Apartments</th>
					</tr>
				</thead>
				        	
				<tbody>
				<c:forEach items="${sessionScope.search_apartments }" var="i">
					<tr class="odd gradeX">
						<td>${i.apartmentId }</td>
						<td>${i.noOfBedroom }</td>
						<td>${i.noOfBathroom }</td>
						<td>${i.aminities }</td>
						<td>${i.address }</td>
						<td>${i.city }</td>
						<td>${i.state }</td>
						<td>${i.zipCode }</td>
						<td>${i.price }</td>
						<td>${i.rentalStatus }</td>
						
						<td>
							<a href="<%=request.getContextPath()%>/addApartmentServlet?flag=edit_apartments&apartment_id=${i.apartmentId}">EDIT</a> 
						</td>
