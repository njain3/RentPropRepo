			...
			<table class="table table-bordered datatable" id="table-1">
				<thead>
					<tr>
						...
						<th><b>Address</b></th>
						<th><b>Manage Residents</b></th>
					</tr>
				</thead>
				        	
				<tbody>
				<c:forEach items="${sessionScope.search_residents }" var="i">
					<tr class="odd gradeX">
						<td>${i.loginDto.username }</td>
						<td>${i.loginDto.firstName }</td>
						<td>${i.loginDto.lastName }</td>
						<td>${i.loginDto.emailId }</td>
						<td>${i.loginDto.phoneNumber }</td>
						<td>${i.address }</td>
						<td>
						<a href="<%=request.getContextPath()%>/addResidentServlet?flag=edit_residents&username=${i.loginDto.username}">EDIT</a> 
						/ <a onclick="return confirmDelete()" href="<%=request.getContextPath()%>/addResidentServlet?flag=delete_resident&username=${i.loginDto.username}">DELETE</a>
						</td>
					</tr>
				</c:forEach>	
				</tbody>
